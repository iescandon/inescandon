...
function MyForm() {
  ...
  return (
      <form className="flex flex-col focus:outline-goldenrod" onSubmit={handleOnSubmit}>
        ...
        <button className="self-end w-1/4 p-3 mt-10 text-white rounded md:transition md:duration-200 md:ease-in-out md:hover:text-black md:hover:bg-goldenrod bg-coolBlue md:w-1/6 submit-btn" type="submit" disabled={serverState.submitting}>
          Submit
        </button>
        {serverState.status && (
