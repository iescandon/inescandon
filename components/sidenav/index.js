...
export default function SideNav({ selectedSection }) {
  ...
  return (
    <nav
      id="sidenav"
      className="relative inset-0 flex items-center justify-center w-full p-5 overflow-auto text-center border-b shadow-md lg:border-r border-silver lg:fixed lg:w-3/12 lg:min-h-screen lg:flex-col lg:shadow-lg"
    >
      <Link href="/">
        <img
          id="logo"
          src="/images/blue-logo.webp"
          alt="blue logo for website"
      ...
      <p className="hidden mt-3 text-5xl font-dawningOfaNewDay lg:block my-name">
        Inez Escandón
      </p>
      <p className="hidden font-semibold uppercase lg:block role">
        Full Stack Web Developer
      </p>
      <ul
        className="flex flex-row ml-auto lg:mt-3 lg:flex-col lg:ml-0"
        id="nav-items"
      >
        {sections.map((item, i) => {
